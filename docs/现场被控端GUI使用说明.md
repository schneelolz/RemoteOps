...
4. 等待控制码获取动画结束，窗口显示“等待工程师连接”和九位临时控制码；
5. 通过可信渠道把本次控制码提供给工程师；
6. 工程师配对后，窗口状态切换为“工程师已连接”；
7. 工作完成后，连接中点击“停止协助”；没有连接时点击“退出 Agent”，或关闭窗口并在确认框中结束本次运行。
...
- 原生标题：随界面语言显示产品名称，并从当前编译包版本自动显示版本号；
- 主状态：显示服务是否就绪，以及启动、连接、自动重连、停止或失败状态；获取首个控制码时显示等待动画；
...
- 连接详情：显示 Relay、进程提升状态和传输目录；传输目录会去除 Windows 扩展路径前缀，点击路径或右侧打开图标可用资源管理器打开；不显示 MCP 的逐项确认、完全控制、Controller 授权或 SSH 凭据明细；
- 结束入口：有工程师连接时显示“停止协助”，空闲时显示“退出 Agent”，点击后复用关闭窗口时的确认流程。

### 实时日志与历史记录

默认只显示“实时日志”入口。点击入口后，日志抽屉从右侧覆盖主界面的一部分；窗口宽度和底层布局保持不变。点击关闭按钮、按 Esc 或点击抽屉外部可收起，收起时不会清除记录。

日志包含已授权命令、开始执行、标准输出/错误输出和执行结果。界面支持筛选、文本选择和内部滚动；查看历史时不会被新日志强制拉回。历史限于当前进程，最多保留最近 1000 条；“清空”只清除界面内存记录，不删除审计文件，也不停止远程任务。

日志复用审计层的常见凭据脱敏规则，不读取请求附带的认证、文件或加密凭据载荷。输出先按完整行合并后脱敏，因此没有换行的输出会在请求结束时显示；超长行和过量输出会显示省略提示。日志是有界的现场诊断视图，不能替代完整审计。
...
- 现场人员需要立即终止当前任务和连接时，点击“停止协助”或直接关闭 Agent 窗口；没有连接时可点击“退出 Agent”。
