...
## Agent GUI 小修正候选（2026-09-08）

Agent GUI 源码升级为 `0.2.0-preview.9`。原生标题从编译包版本动态显示版本号，原生窗口和应用内标题统一使用实际 RemoteOps 品牌图标，应用内标题改为 `RemoteOps Agent` 以避免与原生标题重复。首次获取控制码时显示等待动画；连接详情中的传输目录使用易读路径并可点击打开。本次只调整 GUI 和语言资源，不改变 Agent 运行层、Service 或 v14 协议，未执行远端部署或公开发布。
