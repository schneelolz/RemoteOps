...
- 控制模式行复用盾牌图标，展示 MCP 经 Relay 同步的逐项确认、完全控制、过期、只读或外部审批状态；AI 已连接但旧 Relay 未提供模式时显示“控制模式未同步”。断线或解绑后清除旧状态，该字段仅用于展示，不改变授权。
- `--demo` 使用虚构控制码、模拟已连接状态和控制模式绑定，不建立远端连接；演示与真实连接复用同一显示逻辑。完整模式同步需同时更新 Relay 和 Agent，MCP 主动切换立即上报，空闲过期由现有 15 秒周期检测。
