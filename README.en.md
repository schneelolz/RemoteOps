...
**AI-assisted field diagnostics with explicit connection, permission, approval, and audit boundaries.**
...
> Current status (2026-09-01): the source candidate is `0.2.0-preview.5`; no public GitHub Release has been created yet. This version is for developer evaluation and controlled pilots, not critical production use.

## Current status

The current source version is `0.2.0-preview.5`, the candidate for the first GitHub Technical Preview. Before publishing, clean-checkout builds must produce and verify the Windows, Linux Relay, Windows MCP, and Apple Silicon macOS MCP artifacts. Local `artifacts` directories are not public download sources.
...
Automated checks, core transport, permissions, environment discovery, MCP tools, bilingual GUI, and serial-core regression are in place. Low-privilege Windows, Windows Service, real serial/switch hardware, the complete Apple Silicon installation path, Windows Server/RDP/virtual-machine compatibility, and the Windows/macOS signing strategy remain open validation items.

Support and validation status:

| Component | Preview scope | Current status |
|---|---|---|
| Windows Agent CLI/GUI | Windows x64 | Core and automated tests pass; low-privilege and field-environment retesting remains |
| Windows Agent Service | Windows x64 | Implemented; install, recovery, uninstall, and `LocalService` boundary still require acceptance |
| Relay | Linux x64 + Docker | Core and historical three-machine path pass; release assets must be regenerated by Release CI |
| Controller / MCP | Windows x64, Apple Silicon macOS | Core tests pass; Apple Silicon installation and Codex `/mcp` acceptance remains |
| Serial / switch support | Controlled capabilities | Local serial validation passes; formal remote hardware path remains to be retested |
...
The preview does not provide a shared public Relay or a public download URL yet. Deploy your own Relay first, then obtain matching Agent and MCP packages from source or from the GitHub Release after it is published. Agent, Relay, and MCP must use the same protocol version.
...
If a GitHub Release is not yet available, build a local validation package from source first. Do not treat local `artifacts` as a public download source.
...
After publication, the GitHub Release should contain the Windows Agent/Controller, Windows MCP, Apple Silicon macOS MCP, Linux x64 Relay, project and third-party license manifests, a machine-readable dependency inventory, and `SHA256SUMS.txt`. Rebuild from a clean commit or Release CI and re-check hashes before creating the Release. Windows artifacts are currently unsigned and may trigger SmartScreen warnings; macOS MCP artifacts are also unsigned and not notarized, and the Release notes must disclose that risk.

Recommended release-gate order:

1. Run `cargo fmt`, `cargo check`, `cargo clippy`, `cargo test`, and the documentation-link check.
2. Generate and validate all four release asset types and their SHA-256 sums in CI.
3. Complete the low-privilege Windows, real Apple Silicon Mac, and real serial/switch acceptance checks.
4. Confirm signing policy, changelog, asset allowlist, and rollback notes, then create the `v0.2.0-preview.5` GitHub prerelease.
